...
export async function GET(request) {
  try {
    ...
    // Create secure embed URL with additional parameters for security
    const embedUrl = `https://docs.google.com/presentation/d/${fileId}/embed?start=false&loop=false&delayms=3000&rm=minimal&ui=1&chrome=false&toolbar=0&navpanes=0&scrollbar=0`;
    const fallbackUrl = `https://docs.google.com/presentation/d/${fileId}/preview?rm=minimal&ui=1`;
    ...
    return NextResponse.json({
      ...
      message: 'Secure PPT viewer URL generated successfully'
    });

  } catch (error) {
    console.error('Error in secure-ppt API:', error);
    return NextResponse.json(
...
export async function POST(request) {
  try {
    ...
    // Create secure embed URL with enhanced security parameters
    const embedUrl = `https://docs.google.com/presentation/d/${fileId}/embed?start=false&loop=false&delayms=3000&rm=minimal&ui=1&chrome=false&toolbar=0&navpanes=0&scrollbar=0`;
    const fallbackUrl = `https://docs.google.com/presentation/d/${fileId}/preview?rm=minimal&ui=1`;
    ...
    console.error('Error in secure-ppt POST API:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
